...
<!-- rule:1230 -->
- Keep setup for tree-sitter parsers, ast-grep rule collections, glob matchers, regexes, and parsed config out of hot loops unless it depends on the current item.
<!-- rule:3 -->
...
<!-- rule:1231 -->
- Avoid `.unwrap()` in production paths. Use `.expect("...")` only for checked invariants that cannot fail at runtime, with a message naming that invariant.
<!-- rule:661 -->
...
<!-- rule:1212 -->
- Use tree-sitter nodes inside language adapters to lower syntax-derived facts instead of splitting signatures, scanning lines, or matching source text; keep downstream core/rule logic on shared language-agnostic facts.
<!-- rule:1217 -->
