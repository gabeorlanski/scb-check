...
- **Verbosity** is the **union** of clone SLOC lines, ast-grep SLOC lines, and trivial-wrapper SLOC lines per file, divided by total SLOC. See `_count_union_lines` in [`score.py`](score.py). It is not a sum — lines flagged by multiple sources count once.
...
[`render.render_flags`](render.py) emits flags ordered by `(display_path, line, kind_rank)` where kind_rank is clone=0, ast-grep=1, trivial-wrapper=2, erosion=3, cog_erosion=4. Tests assert on textual markers (e.g. `"duplicate-structure:"`, `"warning[...]:"`, `"trivial-wrapper[...]:"`, `"erosion: function ..."`, `"cog_erosion: function ..."`), not the exact layout — preserve those prefixes.
